...
tag=""
model=""
decode_config="conf/decode_asr_ctc_greedy.yaml"
datasets=()
decode=true
scoring=true
ngpu=1
...
Usage: $0 --tag TAG --model MODEL --decode_config conf/decode_asr_ctc_greedy.yaml --datasets dataset1 [dataset2 ...]
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --datasets)
            ...
            ;;
        --decode)
            decode="$(normalize_bool "$2")"
            shift 2
            ;;
        --scoring)
            scoring="$(normalize_bool "$2")"
            shift 2
            ;;
        --ngpu)
            ngpu="$2"
        ...
        --score_opts)
            ...
            ;;
        --expert_heatmap)
            expert_heatmap="$(normalize_heatmap_policy "$2")"
            shift 2
            ;;
        -h|--help)
            echo "${help_message}"
...
inference_tag="decode_${decode_tag}_${model_tag}"
...
if ${decode}; then
    ...
    for dataset in "${datasets[@]}"; do
        ...
        if [ ! -f "${data_dir}/text" ]; then
            echo "text not found for dataset: ${dataset}" >&2
            exit 1
        fi
        ...
        awk '{print $1}' "${data_dir}/text" > "${keys_file}"
        rm -f "${logdir}"/keys.*.scp "${logdir}"/asr_inference.*.log
        ...
        for split_file in "${logdir}"/keys.*.scp; do
            ...
            log_path="${logdir}/asr_inference.${job_id}.log"
            "${python_bin}" -m inference.asr_inference \
        ...
        if [ "${failed}" -ne 0 ]; then
            grep -l -i error "${logdir}"/asr_inference.*.log 2>/dev/null | while read -r err_log; do
                cat "${err_log}"
            done
            exit 1
        fi
        ...
        if [ "${expert_heatmap}" != "false" ] && [ ${#stats_files[@]} -gt 0 ]; then
            "${python_bin}" -m inference.merge_expert_heatmap \
                ...
                --title "ASR Inference Expert Usage: ${dataset}"
...
if ${scoring}; then
    ...
    for dataset in "${datasets[@]}"; do
        dataset_dir="${decode_root}/${dataset}"
        "${python_bin}" -m inference.asr_score \
            --dataset "${dataset}" \
